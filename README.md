...
## The numbers are enforced, not promised

```bash
make verify
```

Extracts every number carrying a measurement unit from `RESULTS.md`,
`README.md` and `CLAUDE.md`, and fails if it does not trace to a run
artifact. A claim passes only if some artifact value, or some ratio between
two artifact values on the same hardware, rounds to it at the precision it
was written with. Writing 1.96x from 1.963 is fine. Writing 1.8x is not.

It found five unsupported claims the first time it ran. `make gate` runs the
tests and this together.
